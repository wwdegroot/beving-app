...
impl WGS84Coordinate {
    ...
    pub fn to_rdnew(&self) -> RDCoordinate {
        ...
        let rpq: &[f64; 9] = &[
            190094.945, -11832.228, -114.221, -32.391, -0.705, -2.340, -0.608, -0.008, 0.148,
        ];

        let sp: &[f64; 10] = &[1.0, 0.0, 2.0, 1.0, 3.0, 0.0, 2.0, 1.0, 0.0, 1.0];
        let sq: &[f64; 10] = &[0.0, 2.0, 0.0, 2.0, 0.0, 1.0, 2.0, 1.0, 4.0, 4.0];
        let spq: &[f64; 10] = &[
            309056.544, 3638.893, 73.077, -157.984, 59.788, 0.433, -6.439, -0.032, 0.092, -0.054,
        ];
        ...
        for (i, _v) in rpq.iter().enumerate() {
            x += rpq[i] * f64::powf(lat, rp[i]) * f64::powf(lon, rq[i])
        }

        for (i, _v) in spq.iter().enumerate() {
            y += spq[i] * f64::powf(lat, sp[i]) * f64::powf(lon, sq[i])
        }
