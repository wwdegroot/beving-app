...
pub async fn refresh_knmi_data(State(state): State<Arc<AppState>>) {
    ...
    while let Some(_ts) = refresh_interval_timer.next().await {
        ...
        let refresh_bevingen_data = match init_knmi_bevingen().await {
        ...
        if !refresh_bevingen_data.events.is_empty() {
            // update induced data in appstate
            let induced = state.induced.try_write();
            match induced {
                Some(mut induced) => *induced = refresh_bevingen_data.clone(),
                None => {
            ...
            match induced_geojson {
                Some(mut induced_geojson) => *induced_geojson = refresh_bevingen_data.into(),
                None => {
